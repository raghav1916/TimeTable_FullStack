...
router.put('/api/schedules',(req,res)=>{
    ...
    schedule_db.get('schedule').find({schedule_name:schedule.schedule_name}).assign({courses_list:schedule.courses_list}).value();
    schedule_db.write();
    res.send(schedule);
        });
...
router.get('/api/schedulesTimeTable/:schedule_name',(req,res)=>{
    ...
    let c4=data_db.get('data').filter({catalog_nbr:c3[i].catalog_nbr,subject:c3[i].subject}).value();
    timetable.push(c4);
    }
    res.send(timetable);
    //let c2=data_db.get('data').chain().filter({catalog_nbr:req.params.course,subject:req.params.subject}).value();
